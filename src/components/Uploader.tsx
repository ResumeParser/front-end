...
const Uploader = ({ onFileSelect }: UploaderProps) => {
  ...
  return (
    ...
    >
      <div {...getRootProps()} className={`p-8 border-2 border-dashed border-gray-700 rounded-xl text-center transition-colors ${!selectedFile ? 'hover:border-gray-500 cursor-pointer' : ''} ${isDragActive ? 'border-gray-500 bg-gray-900' : ''}`}>
        <input {...getInputProps()} />
        {!selectedFile ? (
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="mt-6 flex space-x-4">
              <button
                onClick={handleReset}
                className="px-6 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 text-white font-semibold transition-colors flex items-center space-x-2"
              >
                <FiRefreshCw size={16} />
              ...
              <button
                ...
                className="px-6 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-semibold transition-colors flex items-center space-x-2"
              >
                <FiPlay size={16} />
                <span>Analyze</span>
              </button>
